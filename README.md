...
- **Workflow analysis and egress control.** The CI/CD workflows
  ...
  on every change, the release pipeline runs under a block-mode egress
  allowlist, and the published tarball's exact file list is asserted
  against a checked-in manifest before every publish.
